...
from technical.indexnow_checker import check_key_in_meta, check_key_file, check_robots_txt
...
class TestIndexNowChecker(unittest.TestCase):
    ...
    def test_key_file_found_and_matches(self):
        case = self.cases[4]
        import technical.indexnow_checker as module
        with patch.object(module, 'fetch_url', return_value=(case["key_file_status"], case["key_file_body"])) as mock_fetch:
    ...
    def test_key_file_exists_but_key_doesnt_match(self):
        case = self.cases[5]
        with patch("technical.indexnow_checker.fetch_url") as mock_fetch:
