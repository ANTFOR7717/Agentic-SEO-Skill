...
class TestGitHubAPI(unittest.TestCase):
    ...
    def test_gh_auth_details_not_available(self):
        with patch("github_api.gh_available", return_value=False):
            result = gh_auth_details(force_refresh=True)
            self.assertFalse(result["available"])
            self.assertFalse(result["authenticated"])

    def test_gh_auth_details_authenticated(self):
        with patch("github_api.gh_available", return_value=True):
            with patch("subprocess.run") as mock_run:
                mock_result = MagicMock()
    ...
    def test_auth_context_with_token(self):
        with patch("github_api.gh_auth_details", return_value={"available": True, "authenticated": False}):
    ...
    def test_auth_context_with_gh(self):
        with patch("github_api.gh_auth_details", return_value={"available": True, "authenticated": True}):
            result = auth_context(token="")
            self.assertEqual(result["mode"], "gh")

    def test_auth_context_unauthenticated(self):
        with patch("github_api.gh_auth_details", return_value={"available": False, "authenticated": False}):
            result = auth_context(token="")
